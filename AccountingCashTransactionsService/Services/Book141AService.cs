...
{
    public class Book141AService : EntityRepositoryCore<Book141A>, IBook141AService
    {
        ...
        public Book141AService(IDbContext contexts, DataContext context) : base(contexts)
        {
            _context = context;
        ...
        /// <returns></returns>
        public ResponseCoreData GeByDate(int bankCode, int userId, DateTime date)
        {
            var result = new Journal109ResultQuerys();
            var dates = date.ToString("dd.MM.yyyy");
            string sqltxt = $"select * from table(GET_BOOK_141A(to_date('{dates}','DD.MM.YYYY'),{bankCode}))";
            var entity = _context.Query<QueryModels>().FromSql(sqltxt).OrderBy(f => f.Id).ToList();
            result.Data = entity;
        ...
        private int SetSaldoBeginProcedure(int bankCode, DateTime date, int saldoBeginCount, double saldoBeginSumma)
        {
            ...
            using (var cmd = _context.Database.GetDbConnection().CreateCommand())
            {
                ...
                cmd.CommandText = "SET_FIRST_SALDO_BOOK141A";
        ...
        public byte[] ToExport(List<ExcelModelForBook141> model, string user, int bankCode)
        {
            ...
            try
            {
                ...
                var ws = excel.Workbook.Worksheets.Add("Book141A");
                ws.Cells["D1"].Value = "141A-Шакл";
